...
////////////////////////////////////////////////////////////////////////////////////////////////////
void PostProcess::CreateComputeSettings(int horizRes, int vertRes)
{
	// Create WRITE-ONLY TEXTURE
	//glGenTextures(1, &m_ComputeBuffer);
	//glBindTexture(GL_TEXTURE_2D, m_ComputeBuffer);
	//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, horizRes, vertRes, 0, GL_RGBA, GL_FLOAT, nullptr);
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	//glBindImageTexture(0, m_ComputeBuffer, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
	//
	//// Get maximum size of total work group for glDispatchCompute()
	//glGetInteger64i_v(GL_MAX_COMPUTE_WORK_GROUP_COUNT, 0, &m_iWorkgroupCount[0]);
	//glGetInteger64i_v(GL_MAX_COMPUTE_WORK_GROUP_COUNT, 1, &m_iWorkgroupCount[1]);
	//glGetInteger64i_v(GL_MAX_COMPUTE_WORK_GROUP_COUNT, 2, &m_iWorkgroupCount[2]);
	//
	//std::string strWorkgroupCount = std::to_string(m_iWorkgroupCount[0]) + " | " + std::to_string(m_iWorkgroupCount[1]) + " | " + std::to_string(m_iWorkgroupCount[2]);
	//UIManager::getInstance().WriteToConsole(LOGTYPE::LOG_DEBUG, "PostProcess", strWorkgroupCount);
	//
	//// Get maximum size of local work group, defined in shader with "layout" qualifier
	//glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_SIZE, 0, &m_iWorkgroupSize[0]);
	//glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_SIZE, 1, &m_iWorkgroupSize[1]);
	//glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_SIZE, 2, &m_iWorkgroupSize[2]);
	//
	//std::string strWorkgroupSize = std::to_string(m_iWorkgroupSize[0]) + " | " + std::to_string(m_iWorkgroupSize[1]) + " | " + std::to_string(m_iWorkgroupSize[2]);
	//UIManager::getInstance().WriteToConsole(LOGTYPE::LOG_DEBUG, "PostProcess", strWorkgroupSize);
	//
	//glGetIntegerv(GL_MAX_COMPUTE_WORK_GROUP_INVOCATIONS, &m_iWorkgroupInvocations);
	//
	//std::string strWorkgroupInvoc = std::to_string(m_iWorkgroupInvocations);
	//UIManager::getInstance().WriteToConsole(LOGTYPE::LOG_DEBUG, "PostProcess", strWorkgroupInvoc);
}
...
void PostProcess::ExecutePostprocessPass()
{
	...
	//m_pPostComputeShader->Use();
	//glDispatchCompute(1280, 800, 1);
